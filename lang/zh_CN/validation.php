...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | 以下语言行包含验证器类使用的默认错误消息。部分规则（如尺寸类规则）
    | 拥有多个版本。你可以在这里自由调整这些消息。
    |
    */
    ...
    'lte' => [
        'array' => ':attribute 必须不超过 :value 个项目。',
    ...
    'password' => [
        ...
        'uncompromised' => '给定的 :attribute 已在数据泄露中出现。请更换不同的 :attribute。',
    ],
    ...
    'starts_with' => ':attribute 必须以下列任一值开头：:values。',
    ...
    |--------------------------------------------------------------------------
    |
    | 这里可按 "attribute.rule" 约定为属性指定自定义验证消息，便于为某个
    | 属性规则快速指定特定的语言行。
    |
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 以下语言行用于将属性占位符替换为更友好的名称（如将 "email" 替换为
    | "邮箱地址"），让消息更易读。
    |
    */
